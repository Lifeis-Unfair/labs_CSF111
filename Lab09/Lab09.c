...
emp max_salary(emp *e1p, emp *e2p) 
{
  
  if(e1p->salary > e2p->salary) return *e1p;
  else if(e1p->salary < e2p->salary) return *e2p;
  else if (e1p->id > e2p->id) return *e1p;
  else return *e2p;
...
void cal_sick_leaves(int *arr,int n, int *modp, double *avgp)
{
  double val=0;
  int ans=0;
  for (int i = 0; i < n; i++)
